 ...
 *      (term_t NVert, term_t Graph,
 *       term_t Min, term_t Max, term_t Maximal,
 *       term_t Order, term_t Weights, term_t Timeout,
 ...
 * @param Weights the weights of vertices 
 *                [pass variable for none]
 * @param Timeout if cputime exceeds Timeout then stop.
 *                timeout is only checked when a clique
 *                is found so this implies that Total > 0
 *                Timeout < 0 if no timeout, Timeout = 0
 *                is the same as NCliques = 1.
...
pl_cliquer_clique_find_n_sols
    (term_t NVert, term_t Graph,
     term_t NCliques, term_t Cliques, term_t Total,
     term_t Options)
    // Options = [term_t Min, term_t Max, term_t Maximal, term_t Order, term_t Weights, term_t Timeout]
{
    term_t Min     = PL_new_term_refs(6) ;
    term_t Max     = Min     + 1 ;
    term_t Maximal = Max     + 1 ;
    term_t Order   = Maximal + 1 ;
    term_t Weights = Order   + 1 ;
    term_t Timeout = Weights + 1 ;
    
    PL_get_list_ex (Options, Min    , Options) ;
    PL_get_list_ex (Options, Max    , Options) ;
    PL_get_list_ex (Options, Maximal, Options) ;
    PL_get_list_ex (Options, Order  , Options) ;
    PL_get_list_ex (Options, Weights, Options) ;
    PL_get_list_ex (Options, Timeout, Options) ;
    ...
    double timeout ;
    if (!PL_get_float_ex (Timeout, &timeout))
	PL_fail ;
...
//     * ((int*) opts.user_data) = 0 ;
    ...
    struct pl_cliquer_user_data* info = (struct pl_cliquer_user_data*) malloc (sizeof (struct pl_cliquer_user_data)) ;
    opts.user_data = info ;
    if (!opts.user_data)
    { graph_free (graph) ;
      PL_fail ;
...
{   struct pl_cliquer_user_data* info ;
    info = (struct pl_cliquer_user_data*) opts->user_data ;
    ...
    if (info->has_timeout)
    { clock_t end = clock () ;
      cputime = ((double) (end - info->start)) / CLOCKS_PER_SEC ;
    }
    
    return (!info->has_timeout || info->timeout > cputime) && 
           (info->cliques < opts->clique_list_length) ;
    
// 	(((struct pl_cliquer_user_data*) opts->user_data)->cliques)++ ;
...
static const PL_extension predicates[] = 
{
    ...
    {"$cliquer_clique_find_n_sols"    , 6, (void*) pl_cliquer_clique_find_n_sols    , 0},
