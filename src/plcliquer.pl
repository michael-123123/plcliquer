 ...
 * @file plcliquer.pl
 * 
 * the toplevel module for pl-cliquer
 * **********************************************
 ...
 * additional options:
 * 	- timeout(number)
 * 	  this is a loose restriction on the time
 * 	  that plcliquer runs. whenever a clique
 * 	  is found plcliquer tests if timeout has
 * 	  been exceeded. if so, the search stops.
 * 	  if timeout=0 this predicate is the same
 * 	  as clique_find_single, timeout < 0
 * 	  means no timeout.
 * **********************************************
 */
clique_find_n_sols(NSols, Graph, Sols) :-
    proper_length(Graph, NVert),
    '$cliquer_clique_find_n_sols'(NVert, Graph, NSols, Sols, _, [0, 0, true, _, _, -1]).
...
clique_find_n_sols(NSols, NVert, Graph, Sols, Total, Opts) :-
    ...
    option(min_weight(Min), Opts, 0),
    option(max_weight(Max), Opts, 0),
    option(maximal(Maximal), Opts, true),
    option(timeout(Timeout), Opts, -1),
    option(static_ordering(Order), Opts, _),
    option(weights(Weights), Opts, _),
    must_be(integer, NVert),
    must_be(integer, Min),
    must_be(integer, Max),
    must_be(boolean, Maximal),
    must_be(integer, Timeout),
    once(var(Order) ; must_be(list, Order)),
    once(var(Weights) ; must_be(list, Weights)), !,
    '$cliquer_clique_find_n_sols'(NVert, Graph, NSols, Sols, Total, [Min, Max, Maximal, Order, Weights, Timeout]).
